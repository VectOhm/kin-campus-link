...
export const Route = createFileRoute("/parent/attendance")({ component: () => {
      ...
      <div className="space-y-4 p-6">
        ...
        <Section title="Daily Log">
          <table className="data-table w-full">
            ...
            <tbody>{att.map((a) => (
              <tr key={a.id}><td className="font-mono text-xs">{a.date}</td><td><Badge tone={a.status === "present" ? "success" : a.status === "absent" ? "danger" : "warning"}>{a.status}</Badge></td></tr>
            ))}</tbody>
          </table>
        </Section>
      </div>
