...
function AttendancePage() {
      ...
      if (existing) {
        return {
          ...
          attendance: s.attendance.map((a) =>
            a.id === existing.id
              ? { ...a, totalDays, presentDays: p, status: "present" as const }
              : a,
          ),
        };
      }
  ...
  function applyTotalToAll() {
    update((s) => ({
      ...s,
      attendance: s.attendance.map((a) =>
        a.month === month && a.classId === classId ? { ...a, totalDays } : a,
      ),
    }));
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Enter total class days and present count per student"
        actions={
          <button
        ...
        <div className="flex flex-wrap items-end gap-2">
          ...
          <label className="flex items-center gap-1 text-xs text-muted-foreground">
            ...
            <input
              ...
              className="w-16 rounded border border-input bg-background px-2 py-1 text-sm tabular-nums"
            />
          </label>
          <button
            onClick={applyTotalToAll}
                  ...
                  <tr key={s.id}>
                    ...
                    <td>
                      <input
                        ...
                        onBlur={(e) =>
                          e.target.value !== "" && setPresent(s.id, Number(e.target.value))
                        }
                    ...
                    <td
                      className={`font-mono text-xs ${pct >= 75 ? "text-success" : pct >= 60 ? "text-warning" : "text-destructive"}`}
                    >
                      {att ? `${pct}%` : "—"}
                    </td>
                  </tr>
