...
export const Route = createFileRoute("/parent/results")({
  component: () => {
    ...
    const overall = myMarks.length
      ? Math.round(myMarks.reduce((s, m) => s + (m.marks / m.maxMarks) * 100, 0) / myMarks.length)
      : 0;
        ...
        <div className="space-y-4 p-6">
          ...
          {state.exams.map((ex) => {
            ...
            const avg = Math.round(
              ms.reduce((s, m) => s + (m.marks / m.maxMarks) * 100, 0) / ms.length,
            );
            return (
              <Section
                key={ex.id}
                title={`${ex.name} · ${ex.term}`}
                actions={<Badge tone="info">Avg {avg}%</Badge>}
              >
                  ...
                  <tbody>
                    {ms.map((m) => {
                      ...
                      return (
                        <tr key={m.id}>
                          <td>{sub?.name}</td>
                          <td className="tabular-nums">{m.marks}</td>
                          <td className="tabular-nums text-muted-foreground">{m.maxMarks}</td>
                          <td>
                            <Badge tone="info">{m.grade}</Badge>
                          </td>
                        </tr>
                      );
                    })}
