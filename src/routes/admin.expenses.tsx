  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Track operational spend, run payroll with tax calculation"
      />
    ...
    <>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button
          ...
          className="ml-auto flex items-center gap-1.5 rounded-md border border-accent/50 bg-accent/10 px-3 py-1.5 text-xs font-medium text-accent hover:bg-accent/20"
        >
...
function ExpenseChatBot({ onClose }: { onClose: () => void }) {
  ...
  const [msgs, setMsgs] = useState<ChatMsg[]>([
    {
      id: "m0",
      from: "bot",
      text: 'Hi! Tell me what to add. Try: "add 550 for utilities" or "spent 2300 on supplies".',
    },
  ]);
  ...
  return (
    <Modal
      open
      onClose={onClose}
      title="Expense Chatbot"
      subtitle="Plain English → expense entry"
      maxWidth="max-w-md"
    >
      <div className="flex h-[400px] flex-col">
        <div className="flex-1 space-y-2 overflow-y-auto pr-1">
          {msgs.map((m) => (
            ...
            >
              <div
                className={`max-w-[80%] rounded-lg px-3 py-1.5 text-sm ${m.from === "user" ? "bg-primary text-primary-foreground" : "bg-muted text-foreground"}`}
              >
                {m.text}
              </div>
            </div>
...
function PayrollPanel() {
  ...
  const months = useMemo(
    () =>
      Array.from(new Set(state.salaries.map((s) => s.month)))
        .sort()
        .reverse(),
    [state.salaries],
  );
    ...
    update(
      (s) => ({
        ...
        salaries: s.salaries.map((sal) =>
          sal.id === id
            ? { ...sal, status: "paid", paidOn: new Date().toISOString().split("T")[0] }
            : sal,
        ),
    ...
    <>
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <Stat
          ...
          tone="success"
          hint={`${paidCount}/${monthSalaries.length} paid`}
        />
      </div>
                      ...
                      <div className="flex gap-1">
                        ...
                        {s.status === "pending" && (
                          <button
                            ...
                            className="rounded bg-success px-2 py-0.5 text-[11px] font-medium text-success-foreground hover:bg-success/90"
                          >
                            Pay
                          </button>
                        )}
...
function EditSalaryModal({ salaryId, onClose }: { salaryId: string; onClose: () => void }) {
  ...
  function submit(e: React.FormEvent) {
    ...
    update(
      (s) => ({
        ...s,
        salaries: s.salaries.map((x) =>
          x.id === salaryId ? { ...x, basic, allowances, deductions, taxRate, net } : x,
        ),
      }),
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      title="Edit salary"
      subtitle={`${state.teachers.find((t) => t.id === sal.teacherId)?.name} · ${sal.month}`}
    >
      <form onSubmit={submit} className="grid grid-cols-2 gap-3">
        <Field
          label="Basic (₹)"
          type="number"
          value={String(basic)}
          onChange={(v) => setBasic(Number(v))}
        />
        <Field
          label="Allowances (₹)"
          type="number"
          value={String(allowances)}
          onChange={(v) => setAllowances(Number(v))}
        />
        <Field
          label="Deductions (₹)"
          type="number"
          value={String(deductions)}
          onChange={(v) => setDeductions(Number(v))}
        />
        <Field
          label="Tax rate (%)"
          type="number"
        ...
        <div className="col-span-2 rounded-md border border-border bg-muted p-3 text-xs">
          <div className="flex justify-between">
            <span>Gross</span>
            <span className="tabular-nums">₹{gross.toLocaleString()}</span>
          </div>
