...
function AnalyticsPage() {
  ...
  const classPerf = state.classes.map((c) => {
    ...
    const avg = ms.length
      ? Math.round(ms.reduce((s, m) => s + (m.marks / m.maxMarks) * 100, 0) / ms.length)
      : 0;
  ...
  const att = state.classes.map((c) => {
    ...
    const pct = a.length
      ? Math.round((a.filter((x) => x.status === "present").length / a.length) * 100)
      : 0;
    ...
    <div>
      ...
      <div className="space-y-4 p-6">
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
          ...
          <Stat
            label="Net"
            value={`₹${(collected - expenses).toLocaleString()}`}
            tone={collected - expenses >= 0 ? "success" : "danger"}
          />
        </div>
