...
function TimetablePage() {
  ...
  const periodNumbers = Array.from(new Set([...slots.map((s) => s.period), 1, 2, 3, 4, 5, 6])).sort(
    (a, b) => a - b,
  );

  function deleteSlot(id: string) {
    update((s) => ({ ...s, timetable: s.timetable.filter((x) => x.id !== id) }), {
      action: "deleted slot",
      entity: id,
    });
  ...
  return (
    <div>
      <PageHeader
        title="Timetable"
        subtitle="Click any cell to edit · system enforces no teacher double-booking"
        actions={
          <select
            value={classId}
      ...
      <div className="space-y-3 p-6">
        {slots.length === 0 ? (
          <EmptyState message="No slots yet — click + to add" icon={Clock3} />
        ) : null}
                          ...
                          {slot ? (
                            <div>
                              <div className="text-xs font-medium">
                                {state.subjects.find((x) => x.id === slot.subjectId)?.name}
                              </div>
                              <div className="text-[10px] text-muted-foreground">
                                {state.teachers.find((x) => x.id === slot.teacherId)?.name}
                              </div>
                              <div className="absolute right-1 top-1 hidden gap-1 group-hover:flex">
                                <button
                                  onClick={() => setEditing({ slot, day: d, period: p })}
                                  className="rounded bg-card p-0.5 text-muted-foreground hover:bg-muted hover:text-foreground"
                                >
                                  <Pencil className="h-3 w-3" />
                                </button>
                                <button
                                  onClick={() => deleteSlot(slot.id)}
                                  className="rounded bg-card p-0.5 text-muted-foreground hover:bg-destructive/10 hover:text-destructive"
                                >
                            ...
                            <button
                              ...
                              className="flex w-full items-center justify-center gap-1 rounded p-1 text-[10px] text-muted-foreground hover:bg-muted hover:text-foreground"
                            >
        ...
        <Section title="How collision detection works">
          <p className="text-xs text-muted-foreground">
            When you save a slot, the system checks every other class on the same day & period. If
            the chosen teacher is already scheduled to teach somewhere else at that time, the save
            is rejected. Time-range overlaps with existing slots in this class are also blocked.
          </p>
        </Section>
      </div>
      {editing && (
...
function SlotModal({
  ...
  const [teacherId, setTeacherId] = useState(
    existing?.teacherId ??
      csa.find((c) => c.subjectId === (existing?.subjectId ?? subjectOptions[0]?.id))?.teacherId ??
      "",
  );
  ...
  function submit(e: React.FormEvent) {
    ...
    if (!subjectId || !teacherId) {
      toast.error("Subject and teacher required");
      return;
    }
    ...
    const teacherClash = state.timetable.find(
      (slot) =>
        slot.id !== existing?.id &&
    ...
    if (teacherClash) {
      ...
      toast.error(
        `Conflict: teacher already in ${otherCls?.name} (${teacherClash.startTime}-${teacherClash.endTime})`,
      );
      return;
    }

    // Same class, same day, overlapping time
    const classClash = state.timetable.find(
      (slot) =>
        slot.id !== existing?.id &&
    ...
    if (classClash) {
      toast.error(
        `Conflict: this class already has a slot ${classClash.startTime}-${classClash.endTime}`,
      );
      return;
    }
    ...
    if (existing) {
      update(
        (s) => ({
          ...s,
          timetable: s.timetable.map((sl) =>
            sl.id === existing.id ? { ...sl, subjectId, teacherId, startTime, endTime } : sl,
          ),
        }),
  ...
  return (
    <Modal
      open
      onClose={onClose}
      title={`${existing ? "Edit" : "Add"} slot · ${day} · Period ${period}`}
      subtitle="Collision check runs on save"
    >
      <form onSubmit={submit} className="grid grid-cols-2 gap-3">
        <SelectField
          ...
          options={
            subjectOptions.length
              ? subjectOptions.map((s) => [s.id, s.name])
        ...
        <SelectField
          ...
          options={state.teachers
            .filter((t) => t.classes.includes(classId))
