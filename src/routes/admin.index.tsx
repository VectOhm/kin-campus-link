      ...
      <div className="grid gap-4 p-6">
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
          <Stat
            ...
            hint={`Across ${state.classes.length} classes`}
          />
          <Stat
            label="Teachers"
            value={state.teachers.length}
            hint={`${state.subjects.length} subjects`}
          />
          <Stat
          ...
          <Stat
            ...
            tone="warning"
            hint={`${state.feePayments.filter((f) => f.status === "pending").length} payments`}
          />
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
          ...
          <Stat
            ...
            value={`${presentPct}%`}
            tone="info"
            hint={`${todayAttendance.length} marked`}
          />
          <Stat
            label="Bus Routes"
            value={state.busRoutes.length}
            hint={`${state.busRoutes.reduce((s, r) => s + r.stops.length, 0)} stops`}
          />
        </div>
          ...
          <Section title="Recent Activity">
            {recentLogs.length === 0 ? (
              <p className="text-sm text-muted-foreground">
                No activity yet. Make a change to populate the log.
              </p>
            ) : (
