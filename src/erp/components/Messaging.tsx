...
export function Messaging({
  ...
  return (
    <div>
      <PageHeader
        title="Messages"
        subtitle={readOnly ? "Read-only — only teachers can post" : undefined}
      />
      <div className="grid gap-3 p-6 md:grid-cols-[240px_1fr]">
        <div className="rounded-md border border-border bg-card">
          ...
          {groups.length === 0 ? (
            <EmptyState message="No groups" icon={MessageSquare} />
          ) : (
            <ul>
              {groups.map((g) => (
                <li key={g.id}>
                  <button
                    ...
                    className={`w-full px-3 py-2 text-left text-sm hover:bg-muted ${activeId === g.id ? "bg-muted" : ""}`}
                  >
        ...
        <div className="flex h-[60vh] flex-col rounded-md border border-border bg-card">
          <div className="flex-1 space-y-2 overflow-y-auto p-4">
            {thread.length === 0 ? (
              ...
              thread.map((m) => (
                <div
                  key={m.id}
                  className={`flex ${m.fromId === currentUser?.id ? "justify-end" : "justify-start"}`}
                >
                  <div
                    className={`max-w-[75%] rounded-md px-3 py-2 text-xs ${m.fromId === currentUser?.id ? "bg-primary text-primary-foreground" : "bg-muted"}`}
                  >
                    {m.fromId !== currentUser?.id && (
                      <div className="mb-0.5 text-[10px] font-semibold text-accent">
                        {m.fromName}
                      </div>
                    )}
                    <div>{m.body}</div>
                    <div
                      className={`mt-1 text-[10px] ${m.fromId === currentUser?.id ? "text-primary-foreground/60" : "text-muted-foreground"}`}
                    >
          ...
          {readOnly ? (
            <div className="border-t border-border p-3 text-center text-[11px] text-muted-foreground">
              You can read messages but not post in this group.
            </div>
          ) : (
